...
class CartPoleDirectCollocation():
    ...
    def differentiate(self, x, u):
        ...
        exprs = self.system.ca_d_state_vars_mx[3::2]
        orig_vars = self.system.ca_state_vars_mx + self.system.ca_control_vars_mx

        new_vars = []
        for i in range(2+2*self.system.num_poles):
            new_vars.append(x[i])
        new_vars.append(u[0])
...
class CartPoleDirectCollocationUltra():
    def __init__(self, N: int, N_collocation: int, system: CartPoleSystem, tolerance=1e-6):
        self.N = N
        self.N_collocation = N_collocation
        self.N_states = system.num_states
