...
class Cart:
    def __init__(
        ...
        self.x_bound_margin = x_bound_margin
...
class StepperMotor:
    def __init__(
        ...
        self.f_bounds = force_bounds
        self.f_margin = force_margin
...
class CartPoleDCMotorSystem:
    ...
    def constraints(self, state: np.ndarray, control: np.ndarray) -> np.ndarray:
        ...
        constraints = np.concatenate([state[:1]-self.state_lower_bound[:1]*(1-self.cart.x_bound_margin), self.state_upper_bound[:1]*(1-self.cart.x_bound_margin)-state[:1], control-self.control_lower_bound*(1-self.motor.Ia_margin), self.control_upper_bound*(1-self.motor.Ia_margin)-control, [Ia-self.motor.Ia_bounds[0]*(1-self.motor.Ia_margin), self.motor.Ia_bounds[1]*(1-self.motor.Ia_margin)-Ia]])
        return constraints
    ...
    # def tensor_differentiate(self, state: torch.Tensor, control: torch.Tensor) -> torch.Tensor:
    #     # x = state[0]
    #     d_x = state[1]
    #     Va = control[-1]
    #     thetas = state[2::2]
    ...
    #     sum1 = (torch.tensor(self.ms)*torch.sin(thetas)*torch.cos(thetas)).sum()
    #     sum2 = (torch.tensor(self.ms)*(torch.tensor(self.ls)/2)*d_thetas**2*torch.sin(thetas)).sum()
    #     sum3 = ((torch.tensor(self.u_ps)*d_thetas*torch.cos(thetas))/(torch.tensor(self.ls)/2)).sum()
    #     sum4 = (torch.tensor(self.ms)*torch.cos(thetas)**2).sum()

    #     Ia = (Va-self.motor.Ke*d_x/self.motor.r)/self.motor.Ra
    #     h2 = (self.g*sum1-7/3*(1/self.motor.r**2*(self.motor.Kt*self.motor.r*Ia-self.motor.Bm*d_x)-sum2-self.cart.u_c*d_x)-sum3)
    #     g2 = (sum4-7/3*(self.M+self.motor.Jm/self.motor.r**2))
    #     dd_x = h2/g2
    #     dd_thetas = 3/(7*torch.tensor(self.ls)/2)*(self.g*torch.sin(thetas)-dd_x*torch.cos(thetas)-torch.tensor(self.u_ps)*d_thetas/(torch.tensor(self.ms)*torch.tensor(self.ls)/2))
        
    #     d_xs = torch.column_stack((d_x, dd_x)).flatten()
    #     d_thetas = torch.column_stack((d_thetas, dd_thetas)).flatten()

    #     d_state = torch.cat((d_xs, d_thetas))
...
class CartPoleStepperMotorSystem:
    def __init__(
        ...
        self.state_lower_bound = np.array([cart.x_bounds[0], motor.v_bounds[0]] + [-2*pi, np.finfo(np.float64).min]*self.num_poles)
        self.state_upper_bound = np.array([cart.x_bounds[1], motor.v_bounds[1]] + [ 2*pi, np.finfo(np.float64).max]*self.num_poles)

        self.control_lower_bound = np.array([np.finfo(np.float64).min])
        self.control_upper_bound = np.array([np.finfo(np.float64).max])
    ...
    def constraints(self, state: np.ndarray, control: np.ndarray) -> np.ndarray:
        ...
        constraints = np.array([s-self.state_lower_bound[0]*(1-self.cart.x_bound_margin), self.state_upper_bound[0]*(1-self.cart.x_bound_margin)-s, d_s-self.state_lower_bound[1]*(1-self.motor.v_margin), self.state_upper_bound[1]*(1-self.motor.v_margin)-d_s, f-self.motor.f_bounds[0], self.motor.f_bounds[1]-f])
        return constraints
    ...
    def clip(self, state: np.ndarray, control: np.ndarray) -> tuple[np.ndarray,np.ndarray]:
        ...
        f_max = self.motor.f_bounds[1]
        f_min = self.motor.f_bounds[0]
